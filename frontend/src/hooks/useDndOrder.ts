...
interface PersistControlContext {
...
export function useDndOrder<T>(
  ...
  const persistContext = useMemo<PersistControlContext>(
    () => ({ runtimeRef, currentOrderRef, setCurrentOrder }),
    [setCurrentOrder]
  );
  ...
  const onDragEnd = useCallback(
    (e: DragEndEvent) => {
      ...
      onPersistRef.current(
        newOrder,
        createPersistControls(persistContext, {
          persistSeq,
          previousOrder,
          newOrder,
        })
    ...
    [persistContext, setCurrentOrder]
  );
...
function createPersistControls(
  { runtimeRef, currentOrderRef, setCurrentOrder }: PersistControlContext,
  { persistSeq, previousOrder, newOrder }: PersistChange
): PersistControls {
