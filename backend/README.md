...
4. **外部 webhook**（`X-Api-Key`）—— 第三方推任意 PNG / markdown 进 EPD
5. **生产部署托管 frontend dist**（`@fastify/static` + SPA fallback）

技术栈：Bun runtime、Fastify、`@nestjs/platform-fastify`、`nestjs-zod` 校验、`nestjs-pino` 日志、Prisma 7（`@prisma/adapter-mariadb`）直连 MySQL、`sharp` 渲染图片、`bcryptjs` 密码、`jsonwebtoken` JWT。
...
`DeviceState` 包含 `device.{id, mac, name, server_time}` + `group.{id, etag, frame_count, default_frame_seq, position{current, total}}` + `poll_interval_s`。`group` 为 `null` 表示未选。

### Webhook 推送（JWT 或 `X-Api-Key: <WEBHOOK_API_KEY>`）
...
| webhook render | `@Public()` + `@UseGuards(JwtOrApiKeyAuthGuard)` | JWT 或 `X-Api-Key` |
| `/healthz` | `@Public()` | 无 |
...
| key | 默认 | 备注 |
|---|---|---|
| `DATABASE_URL` | —— | `mysql://user:pwd@host:3306/db`，必填 |
| `PORT` | 3001 | |
| `JWT_SECRET` | —— | 必填，≥ 16 字符；`openssl rand -hex 64` 生成 |
| `JWT_EXPIRATION` | `7d` | |
| `WEBHOOK_API_KEY` | —— | 必填，≥ 8 字符；webhook 渲染端点的 `X-Api-Key` |
| `BLOB_DIR` | `./blobs` | docker 镜像里固定 `/data/blobs`（用 volume 挂主机 `./slate/blobs/`） |
| `DEVICE_POLL_INTERVAL_SECONDS` | 60 | 写进 `DeviceState.poll_interval_s` |
| `CORS_ORIGIN` | `http://localhost:5173` | dev frontend；生产同源部署可不设 |
| `NODE_ENV` | `development` | dev 时启用 pino-pretty |
| `LOG_LEVEL` | `info` | |
...
`bun test` 跑完所有 `*.test.ts`。当前覆盖 `render.service.test.ts` —— sharp 管线端到端（输入 PNG，输出 15000 字节 1bpp packed，比对预期 hash）。`prismock` 已装，需要时可在 service 层 mock prisma。
