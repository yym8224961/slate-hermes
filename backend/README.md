...
│   └── blob/                BlobService：{BLOB_DIR}/{groupId}/{frameId}.{img,pcm}
├── common/
...
└── modules/
    ...
    ├── frames/              /groups/:gid/frames + manifest + multipart 解析
...
Group   id name etag owner_user_id? sort_order
        └ frames: Frame[]（cascade delete）

Frame   id (group_id, sort_order) unique 复合键
        caption? image_etag image_size audio_etag? audio_size?
```
...
- **Group.etag** = 所有 frame etag 拼接后 hash，便于固件 manifest 304
- **Frame** 唯一键 `(group_id, sort_order)`，删/重排时整组重写 sort_order
...
DELETE /api/v1/groups/:gid                     删相册（cascade 删 frames + blobs）

POST   /api/v1/groups/:gid/frames                  multipart 创建，append 到末尾（image 必填）
PUT    /api/v1/groups/:gid/frames/order            批量重排
PATCH  /api/v1/groups/:gid/frames/:seq             multipart 部分更新，或 JSON 只改 caption
...
```
GET /api/v1/groups/:gid/manifest                  {group_id, group_etag, frames[], default_frame_seq}
...
GET /api/v1/groups/:gid/frames/:seq/audio         16 kHz mono s16le raw PCM
```
...
```ts
{
  ...
  group:  { id, etag, frame_count, default_frame_seq, sort_order, position: {current, total} } | null
}
```
...
├── {groupId}/{frameId}.img         1bpp packed，15000 字节
├── {groupId}/{frameId}.pcm         16 kHz mono s16le raw PCM
└── render-cache/{key0..2}/{key}.bin
                                    sharp 渲染产物 key = sha1(sourceEtag|w|h|threshold|mode|...)
                                    两层 hex 前缀分桶避免单目录爆 inode
```

ETag 算法：`computeETag(buf) = sha256(buf).slice(0, 16)`。manifest 的 `group_etag` 是所有 frame.image_etag 拼接后再 hash。
...
输出字节序与 firmware `epd_ssd1683.cc` 对齐：MSB-first，bit=1 为白 / bit=0 为黑。前端 `FrameEditor` 用同一份 `shared.preprocess + shared.dither` 在浏览器里预览，确保所见即所得。
