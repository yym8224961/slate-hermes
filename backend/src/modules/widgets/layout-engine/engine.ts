import type { Block, BlockRender, LayoutCtx, WidgetLayout } from './types';
...
/**
 * 渲染完整 widget layout → SVG 字符串。
 * 严格按 layout.size 输出 viewBox；根容器隐式是 vertical_stack（从上往下流式）。
 * 调用方负责把字符串喂给 sharp 或 librsvg。
 */
export function renderLayout(layout: WidgetLayout, ctx: LayoutCtx): string {
  ...
  // 白底（widget 模板自带），确保 1bpp dither 时背景为白。
  return (
