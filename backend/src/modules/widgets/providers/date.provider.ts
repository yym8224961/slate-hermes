import { Injectable } from '@nestjs/common';
import { Solar } from 'lunar-typescript';
import { DateConfig, type DateConfigT } from 'shared';
import type { DataProvider, WidgetFetchCtx } from '../widget-types';

export interface DateProviderData {
  /** 公历：2026 */
  year: string;
  /** 公历月：5 */
  month: string;
  /** 公历日：15 */
  day: string;
  /** 公历月/日，左零：05 / 13 */
  monthDay: string;
  /** 星期，中文：周三 */
  weekdayCN: string;
  /** 农历日期，含闰月：丙午年 三月廿七 / null 表示不显示 */
  lunar: string | null;
  /** 农历日期，不含干支年：农历三月廿七 */
  lunarDate: string | null;
  /** 干支年：丙午年 */
  ganzhiYear: string | null;
  /** 干支月：癸巳月 */
  ganzhiMonth: string | null;
  /** 干支日：己丑日 */
  ganzhiDay: string | null;
  /** 节气名，命中当天才有；否则 null */
  solarTerm: string | null;
  /** 下一个节气，今日命中时与 solarTerm 一致。 */
  nextSolarTerm: string | null;
  /** 距下一个节气的天数；今日命中为 0。 */
  nextSolarTermDays: number | null;
  yi: string[];
  ji: string[];
}

const WEEKDAY_CN = ['星期日', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六'];

/**
 * 把 Date 按 tz 拆成本地 y/m/d。Node 默认 UTC 是大坑，必须明示 tz。
 *
 * 用 Intl.DateTimeFormat 提取分量；非 v8 引擎也应当支持。
 */
function dateInTz(now: Date, tz: string): { y: number; m: number; d: number; weekday: number } {
  const fmt = new Intl.DateTimeFormat('en-US', {
    timeZone: tz,
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
    weekday: 'short',
  });
  const parts = fmt.formatToParts(now);
  const obj: Record<string, string> = {};
  for (const p of parts) obj[p.type] = p.value;
  // weekday short: Sun/Mon/...; 转 0-6（周日=0）
  const wkMap: Record<string, number> = { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 };
  return {
    y: parseInt(obj.year!, 10),
    m: parseInt(obj.month!, 10),
    d: parseInt(obj.day!, 10),
    weekday: wkMap[obj.weekday!] ?? 0,
  };
}

@Injectable()
export class DateProvider implements DataProvider<DateConfigT, DateProviderData> {
  readonly type = 'date';

  validateConfig(raw: unknown): DateConfigT {
    return DateConfig.parse(raw);
  }

  fetchData(config: DateConfigT, ctx: WidgetFetchCtx): Promise<DateProviderData> {
    return Promise.resolve(this.computeData(config, ctx));
  }

  private computeData(config: DateConfigT, ctx: WidgetFetchCtx): DateProviderData {
    const { y, m, d, weekday } = dateInTz(ctx.now, config.tz);
    const solar = Solar.fromYmd(y, m, d);
    const lunar = solar.getLunar();

    const showLunar = config.show_lunar ?? true;
    const showSolarTerm = config.show_solar_term ?? true;

    const ganzhiYear = `${lunar.getYearInGanZhiExact()}年`;
    const ganzhiMonth = `${lunar.getMonthInGanZhiExact()}月`;
    const ganzhiDay = `${lunar.getDayInGanZhiExact()}日`;
    const lunarDate = `农历${lunar.getMonthInChinese()}月${lunar.getDayInChinese()}`;
    const lunarStr = showLunar ? `${ganzhiYear} ${lunarDate.replace(/^农历/, '')}` : null;
    // 节气：getJieQi() 返回最近节气，需要校验是否就是今日
    const jieQi = lunar.getJieQi(); // 如 "立夏"
    const jieQiDate = lunar.getJieQiTable()[jieQi];
    const isToday =
      showSolarTerm &&
      jieQi &&
      jieQiDate &&
      jieQiDate.getYear() === y &&
      jieQiDate.getMonth() === m &&
      jieQiDate.getDay() === d;

    const nextTerm = showSolarTerm
      ? isToday
        ? { name: jieQi, days: 0 }
        : findNextSolarTerm(lunar, y, m, d)
      : null;

    return {
      year: String(y),
      month: String(m),
      day: String(d),
      monthDay: `${String(m).padStart(2, '0')}/${String(d).padStart(2, '0')}`,
      weekdayCN: WEEKDAY_CN[weekday]!,
      lunar: lunarStr,
      lunarDate: showLunar ? lunarDate : null,
      ganzhiYear: showLunar ? ganzhiYear : null,
      ganzhiMonth: showLunar ? ganzhiMonth : null,
      ganzhiDay: showLunar ? ganzhiDay : null,
      solarTerm: isToday ? jieQi : null,
      nextSolarTerm: nextTerm?.name ?? null,
      nextSolarTermDays: nextTerm?.days ?? null,
      yi: lunar.getDayYi().slice(0, 5),
      ji: lunar.getDayJi().slice(0, 5),
    };
  }
}

function findNextSolarTerm(
  lunar: ReturnType<ReturnType<typeof Solar.fromYmd>['getLunar']>,
  y: number,
  m: number,
  d: number
): { name: string; days: number } | null {
  const currentKey = y * 10000 + m * 100 + d;
  const currentTime = Date.UTC(y, m - 1, d);
  const items = Object.entries(lunar.getJieQiTable())
    .map(([name, solar]) => ({
      name,
      key: solar.getYear() * 10000 + solar.getMonth() * 100 + solar.getDay(),
      days: Math.max(
        0,
        Math.round(
          (Date.UTC(solar.getYear(), solar.getMonth() - 1, solar.getDay()) - currentTime) /
            86_400_000
        )
      ),
    }))
    .filter((item) => item.key >= currentKey)
    .sort((a, b) => a.key - b.key);
  const next = items[0];
  return next ? { name: next.name, days: next.days } : null;
}
