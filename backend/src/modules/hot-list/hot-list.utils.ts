...
export function withRanks(items: Array<Omit<HotListItem, 'rank'> | HotListItem>): HotListItem[] {
  return items
    .map((item, index) => {
      const rank = (item as HotListItem).rank;
      const normalized: HotListItem = {
        rank: typeof rank === 'number' && Number.isFinite(rank) ? rank : index + 1,
        title: stripHtml(item.title),
      };
      const hot = item.hot ? stripHtml(item.hot) : '';
      if (hot) normalized.hot = hot;
      const desc = item.desc ? stripHtml(item.desc) : '';
      if (desc) normalized.desc = desc;
      const author = item.author ? stripHtml(item.author) : '';
      if (author) normalized.author = author;
      const url = safeExternalUrl(item.url);
      if (url) normalized.url = url;
      const timestamp = item.timestamp ? String(item.timestamp) : '';
