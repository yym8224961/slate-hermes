/**
 * Dynamic content JSON layout DSL —— 后端读 definitions/*.json 转换的内部表示。
 *
 * 设计原则：
...
export interface DynamicContentLayout {
  size: [number, number]; // 当前固定 [400, 300]
  padding?: number; // 水平 + 底部内边距（px）
  top_offset?: number; // 顶部偏移（保留给非全屏 动态内容布局使用）
  body: Block[];
}

export interface DynamicContentDefinition {
  type: string;
  default_ttl_sec: number | null; // null = push-only（dashboard）
  layout: DynamicContentLayout;
}
