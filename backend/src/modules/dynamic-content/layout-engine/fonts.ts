...
import { VECTOR_FONT_DIR } from '../../../infra/assets/asset-paths';

/** 生成 SVG <style> @font-face 块，将 resvg 字体解析锁定到 backend/assets/fonts/vector。
 *  结果缓存：进程生命周期内只计算一次。
 */
...
export function fontFaceStyle(): string {
  ...
  const faces = fonts.map(({ family, file, weight = 'normal' }) => {
    const p = resolve(VECTOR_FONT_DIR, file);
