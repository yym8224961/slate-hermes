...
import { absoluteUrl, cleanText, compactHot, normalizeTimestamp, withRanks } from '../text';
    ...
    async fetch(signal) {
      ...
      return withRanks(
        data
          ...
          .map((item) => {
            const text = cleanText(item.data?.title || item.data?.content || '');
...
function biliVideoItem(video: BiliVideo): Omit<HotListItem, 'rank'> {
  return {
    ...
    hot: compactHot(video.stat?.view, '观看'),
...
function decodeGbk(value: ArrayBuffer): string {
  return new (TextDecoder as unknown as new (label: string) => TextDecoder)('gb2312').decode(value);
}
