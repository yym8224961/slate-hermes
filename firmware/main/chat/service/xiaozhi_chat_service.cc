...
#include "xiaozhi_message_handler.h"
#include "xiaozhi_settings.h"

namespace {
constexpr char kTag[] = "XiaoChat";

bool ConversationMayRun(xiaozhi::ChatPhase phase) {
    return phase == xiaozhi::ChatPhase::kStarting || phase == xiaozhi::ChatPhase::kRunning;
}

bool ConversationBlocksSleep(xiaozhi::ChatPhase phase) {
    return phase != xiaozhi::ChatPhase::kIdle;
}

bool SetStoppingIfMayRun(std::atomic<xiaozhi::ChatPhase>& phase) {
    auto current = phase.load(std::memory_order_relaxed);
    while (ConversationMayRun(current)) {
        if (phase.compare_exchange_weak(current, xiaozhi::ChatPhase::kStopping, std::memory_order_relaxed))
            return true;
    }
    return false;
}

bool IsStopOrRestartPending(xiaozhi::ChatPhase phase) {
    return phase == xiaozhi::ChatPhase::kStopping || phase == xiaozhi::ChatPhase::kStartPending;
}
}  // namespace
...
void ChatService::StartConversationTask() {
    ...
    std::lock_guard<std::mutex> task_lock(conversation_task_mutex_);
    const ChatPhase             phase = chat_phase_.load(std::memory_order_relaxed);
    if (tasks_.conversation_task || IsStopOrRestartPending(phase)) {
