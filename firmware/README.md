...
nvs_flash_init + cache::Init（LittleFS mount "storage"）
  ...
  → SleepManager.Init（IDLE_DEEP_SLEEP_MIN 默认 5 分钟，captive portal / unbound grace 期禁用）
...
- **bound 状态**：用户在「设置 → 同步频率」中选定（30s / 1m / 5m / 10m / 15m / 30m / 1h，默认 1m），存 NVS `slate/poll_sec`，详见 `net/poll_interval_store.cc`。
- **unbound 状态**：固定 10s 快轮询，让 Web 端输码后 ≤10s 屏切「等待相册」。这段窗口 `SleepManager` 禁 deep sleep；窗口最长 24h，超时或电量 < 20% 后回退正常省电策略，避免长期未配对的设备耗光电池。
...
captive portal 期间 `SleepManager` 禁用 deep sleep，避免用户配网时设备睡着。设备绑定前（unbound）也走同样的禁睡策略，让 Web 端输码后 ≤10s 完成屏切；窗口 24h 或电量 < 20% 兜底解除，详见同步协议章节。
